...
复权参数 `adjust`：`none`（不复权）/ `qfq`（前复权，默认）。前复权由原始 K 线和公司行为在服务端计算，不依赖 AKShare 提供复权 K 线。

K 线 `ts` 对外统一返回 ISO 8601 带时区字符串。分钟线在数据库中保存真实 UTC 时刻；日线用 UTC 00:00 表示对应交易日，避免跨时区改变交易日。
